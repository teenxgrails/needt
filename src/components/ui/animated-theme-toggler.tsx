...
/** Runs the same circular View Transition used by the toggler for theme
 * changes initiated by menus and settings pickers. */
export function animateThemeTransition(
  applyTheme: () => void,
  { duration = 400, fromCenter = true, origin }: ThemeTransitionOptions = {}
) {
  ...
  const x = origin?.x ?? (fromCenter ? window.innerWidth / 2 : 0);
  const y = origin?.y ?? (fromCenter ? window.innerHeight / 2 : 0);
  ...
  root.dataset.magicuiThemeVt = "active";
  const cleanup = () => delete root.dataset.magicuiThemeVt;
