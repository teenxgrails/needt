...
- 2026-07-22: The current completion target is a stable Needt v0.1 personal beta, not another feature expansion. The feature set is frozen until one production golden-loop smoke and final engineering gate pass; onboarding and public-launch work follow afterward.
- 2026-07-22: Needt v0.1 production uses Coolify on the existing VPS with separate web and BullMQ worker services from the same Dockerfile/SHA, plus Coolify-managed PostgreSQL and Redis. Vercel and Neon are not part of the current release path.
- 2026-07-22: Creem billing remains in the product but is not a blocker for the v0.1 personal beta. The release gate prioritizes the core planning loop; purchase/webhook acceptance belongs to the public-launch gate.
- 2026-07-22: The ambient AI companion is included in the v0.1 release candidate and stays enabled by default only if production smoke shows no navigation, accessibility, or performance regression.
