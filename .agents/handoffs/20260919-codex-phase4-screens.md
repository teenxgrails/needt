...
- Phase D, part one (2026-09-25, Claude while Codex is rate-limited): committed the saved views and project health restoration (`1590376`), the `/mail` route fix from the side worktree (`b454f2b`), and the E2E repairs (`951a30c`). The E2E suite was finally executed rather than listed: the workspace journey targeted a button where the row renders a `checkbox` and a `button` where the filter is a `tab`; the Documents journey asserted a settled URL although the editor bounces an unmocked Page back to the list. `playwright.config.ts` now allows 90s per test and 60s per navigation because the suite runs against `next dev` and pays for each route's first compile.
- CI on `951a30c`: `e2e` green for the first time since the port, `quality-gates`, `schema-drift` and `changes` green, `security` still waiting on PR #37.
- The full suite is unreliable on this Mac under load — different specs time out on each run and every one of them passes when run alone. Linux CI is the authority.
- Phase C shipped as draft PR #43 against `landing`: the served `design-refs/landing/index.html` now reads Pro $7, Lifetime $149 with no struck-through price, and "first 300 people". The React path named in the goal exists only on the unmerged `codex/landing-react`.
