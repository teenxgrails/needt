...
- Owner/Coolify action is required before any push: confirm Source commit availability is saved as Available during build for needt, need-worker, and needt-collaboration. Web has already demonstrated a real build SHA, but the collaboration health domain must be restored/attached and serve `/health` over TLS before the public runtime-parity gate can pass.

## Next action

- Owner: confirm the setting in all three Coolify resources; restore the collaboration domain/DNS/TLS or update the Production collaboration-health secret to the actual public `/health` origin; then merge/push this branch. After auto-deploy, verify web's matching non-empty `workerBuildSha` and collaboration's matching `/health` SHA.
